...
install_head() {
    ...
    echo "开始安装依赖包..."
    for p in make git npm openssl openssl-devel; do
        myum $p
    done

    echo "将head插件部署到${INSTALL_DIR}目录下"
    # 或者下载插件：git clone git://github.com/mobz/elasticsearch-head.git
    tar -xf ${PACKAGE_DIR}/elasticsearch-head-5.X-offline.tar.gz -C ${INSTALL_DIR}
    check_ok

    echo "解压node到${INSTALL_DIR}目录下"
    tar -xf ${PACKAGE_DIR}/node-v6.11.2-linux-x64-offline.tar.gz -C ${INSTALL_DIR}
    check_ok
...
install_sql(){
    ...
    echo "解压SQL插件包"
    tar -xf ${PACKAGE_DIR}/elasticsearch-sql-5.5.2.0-offline.tar.gz -C ${INSTALL_DIR}/${ES_DIR_NAME}/plugins/
    check_ok
